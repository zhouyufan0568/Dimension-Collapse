...
public class Player : Photon.PunBehaviour
{
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        if (hit.gameObject.name=="Map"&&!m_Jump&& m_CharacterController.isGrounded&&!m_PreIsJump)
        {
            ...
            {
                //Debug.Log("stair under player");
                return;
            }
            Vector3 forward = new Vector3(hit.transform.position.x-transform.position.x,0, hit.transform.position.z - transform.position.z);
            if(Physics.Raycast(mCenter.position,forward,0.5f))
            {
                return;
            }
            m_Jump = true;
